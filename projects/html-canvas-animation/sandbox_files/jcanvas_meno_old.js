...
$.fn.drawBezier = function(args) {
	...
	for (e=0; e<this.length; e+=1) {
		...
		while (1) {
			...
			lcx2 = params['cx' + (lc+1)];
			lcy2 = params['cy' + (lc+1)];
			if (lx >=0 && ly >=0 && lcx1 >=0 && lcy1 >=0 && lcx2 >=0 && lcy2 >=0) {
...
jC.retrofit = function(retro) {
	if (retro === undefined) {retro = true;}
	// Undo retrofit
	if (retro === false) {
		delete $.fn.drawQuadCurve;
		delete $.fn.drawBezierCurve;
		delete $.fn.canvasDefaults;
		delete $.fn.canvas;
	// Retrofit
	} else {
